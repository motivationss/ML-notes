...
def temporal_fc_backward(dout, cache):
    ...
    N, T, M = dout.shape
    D = x.shape[2]
    dx = np.dot(dout, w.T)   # W and dout
    db = np.sum(dout.reshape(N*T, M), axis=0)                   # dout ++
    dw = np.dot(x.reshape(N*T, D).T, dout.reshape(N*T, M))      # x and dout
    # dw = np.dot(x.T, dout).reshape(D, M)
