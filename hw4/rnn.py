...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        forward_fc1, cache_fc1 = fc_forward(features, W_proj, b_proj)
        word_embed, cache_embed = word_embedding_forward(captions_in, W_embed)
        word_embed = word_embed.transpose(1, 0, 2)
        h, cache_rnn_forward = rnn_forward(word_embed, forward_fc1, Wx, Wh, b)
        h = h.transpose(1, 0, 2)
        forward_fc2, cache_fc2 = temporal_fc_forward(h, W_vocab, b_vocab)
        loss, dx_final = temporal_softmax_loss(forward_fc2, captions_out, mask)
        dx_fc_2, grads['W_vocab'], grads['b_vocab'] = temporal_fc_backward(dx_final, cache_fc2)
        dx_fc_2 = dx_fc_2.transpose(1, 0, 2)
        dx_rnn, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dx_fc_2, cache_rnn_forward)
        dx = dx_rnn.transpose(1, 0, 2)
        grads['W_embed'] = word_embedding_backward(dx, cache_embed)
        dx_fc1, grads['W_proj'], grads['b_proj'] = fc_backward(dh0, cache_fc1)
