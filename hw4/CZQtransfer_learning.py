...
def train_model(device, dataloaders, dataset_sizes, model, criterion,
    ...
    if num_epochs == 0:
        ...
        for inputs, labels in dataloaders['val']:
            inputs = inputs.to(device)   # Copy all variables to GPU by device
            labels = labels.to(device)   # Get the labels of data

            # forward
            outputs = model(inputs)   # The output of the model
            _, preds = torch.max(outputs, 1)   # Pick the maximal value of outputs and 1

            # statistics
            running_corrects += torch.sum(preds == labels.data)   # Count the number of correct predictions

        best_acc = running_corrects.double() / dataset_sizes['val']   # Calculate the accuracy

    # Training for num_epochs steps
    for epoch in range(num_epochs):   # train for 25 epochs here
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                ####################################################################################
                    ...
                    # backward + optimiza only if in training phase
                    if phase == 'train':
                        loss.backward()
...
def visualize_model(device, dataloaders, model, class_names, num_images=6):
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(dataloaders['val']):
            ...
            outputs = model(inputs)
            _,preds = torch.max(outputs,1)   # We dont't care the magnitude of maximum value, so use "_" here
            #preds = ...
